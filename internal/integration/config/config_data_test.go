...
func TestLoadOptions_whenConfigDataProvided_shouldDecodeWithoutFilesystem(t *testing.T) {
	options, result, err := LoadOptions(
		context.Background(),
		WithConfigData("classpath:logging/goark-log.yml", []byte(`
appenders:
  console:
    type: console
root:
...
func TestLoadOptions_whenConfigDataNameHasUnsupportedExtension_shouldReject(t *testing.T) {
	data := []byte("root.level=info")
	if _, _, err := LoadOptions(
		context.Background(),
		WithConfigData("logging.conf", data),
	); err == nil {
