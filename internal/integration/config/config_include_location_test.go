...
func TestNativeLogger_whenAppenderRefIncludeLocationDisabled_shouldClearOnlyThatAppender(
	t *testing.T,
) {
	...
	assertRecordingAppenderCaller(
		t,
		locationAppender,
		"TestNativeLogger_whenAppenderRefIncludeLocationDisabled",
	)
...
func TestLoadOptions_whenIncludeLocationConfigured_shouldPopulateRootLoggerAndAppenderRef(
	t *testing.T,
) {
...
`)
	...
	if len(options.Root.AppenderRefControls) != 1 ||
		options.Root.AppenderRefControls[0].IncludeLocation == nil ||
		*options.Root.AppenderRefControls[0].IncludeLocation {
		t.Fatalf(
			"root appender ref includeLocation = %+v, want explicit false",
			options.Root.AppenderRefControls,
		)
	}
	if len(options.Loggers) != 1 || options.Loggers[0].IncludeLocation == nil ||
		!*options.Loggers[0].IncludeLocation {
		t.Fatalf("logger includeLocation = %+v, want explicit true", options.Loggers)
	}
	if len(options.Loggers[0].AppenderRefControls) != 1 ||
		options.Loggers[0].AppenderRefControls[0].IncludeLocation == nil ||
		!*options.Loggers[0].AppenderRefControls[0].IncludeLocation {
		t.Fatalf(
			"logger appender ref includeLocation = %+v, want explicit true",
			options.Loggers[0].AppenderRefControls,
		)
	}
}

func TestLoadOptions_whenPropertiesAppenderRefIncludeLocationConfigured_shouldPopulateControls(
	t *testing.T,
) {
...
`)
	...
	if len(options.Root.AppenderRefControls) != 1 ||
		...
		*options.Root.AppenderRefControls[0].IncludeLocation {
		t.Fatalf(
			"properties root appender refs = %+v, want explicit false",
			options.Root.AppenderRefControls,
		)
	}
	if len(options.Loggers) != 1 || options.Loggers[0].IncludeLocation == nil ||
