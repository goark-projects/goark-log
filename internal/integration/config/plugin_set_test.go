...
func TestPluginSet_whenRegistered_shouldExposeCoreExtensionPoints(t *testing.T) {
	...
	registrar := NewPluginSet(
		...
		WithPluginJSONTemplateResolver(
			"setConstant",
			func(config JSONTemplateResolverBuildConfig) (JSONTemplateResolver, error) {
				var value string
	...
	template, err := NewJSONTemplateLayout(
		`{"value":{"$resolver":"setConstant","value":"ok"}}`,
