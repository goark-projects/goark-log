...
func TestHandler_whenRootThresholdFilterDeniesDebug_shouldDropBeforeAppender(t *testing.T) {
	...
	if strings.Contains(out.String(), "hidden debug") ||
		!strings.Contains(out.String(), "visible info") {
...
func TestHandler_whenLoggerRegexFilterDeniesMessage_shouldKeepOtherEvents(t *testing.T) {
	...
	if strings.Contains(out.String(), "health probe") ||
		!strings.Contains(out.String(), "request done") {
...
func TestFilteredAppender_whenAttrFilterDenies_shouldOnlyAffectWrappedAppender(t *testing.T) {
	...
	filtered, err := NewFilteredAppender(
		NewConsoleAppender(
			WithConsoleName("filtered"),
			WithConsoleWriter(&filteredOut),
	...
	handler, err := NewHandler(Options{
		Appenders: []Appender{
			filtered,
			NewConsoleAppender(
				WithConsoleName("plain"),
				WithConsoleWriter(&plainOut),
				WithConsoleLayout(TextLayout{}),
			),
		},
	...
	if strings.Contains(filteredOut.String(), "audit event") ||
		!strings.Contains(filteredOut.String(), "business event") {
		t.Fatalf("filtered appender output is wrong: %q", filteredOut.String())
	}
	if !strings.Contains(plainOut.String(), "audit event") ||
		!strings.Contains(plainOut.String(), "business event") {
