...
func TestRollingFileAppender_whenCronScheduleReached_shouldRoll(t *testing.T) {
	...
	if err := appender.Append(
		context.Background(),
		TestEvent("after-cron", start.Add(time.Second)),
	); err != nil {
	...
	if !strings.Contains(string(archiveContent), "before-cron") ||
		!strings.Contains(string(activeContent), "after-cron") {
		t.Fatalf(
			"cron rolling split is wrong, archive=%q active=%q",
