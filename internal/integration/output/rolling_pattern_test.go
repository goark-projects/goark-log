...
func TestRollingFileAppender_whenFilePatternConfigured_shouldArchiveWithDateAndIndex(t *testing.T) {
	...
	for index := 0; index < 3; index++ {
		if err := appender.Append(
			context.Background(),
			TestEvent("pattern-"+strings.Repeat("x", 80), now),
		); err != nil {
...
func TestRollingFileAppender_whenPatternRestarts_shouldContinuePatternIndex(t *testing.T) {
	...
	for index := 0; index < 2; index++ {
		if err := appender.Append(
			context.Background(),
			TestEvent("before-"+strings.Repeat("x", 80), now),
		); err != nil {
	...
	if err := appender.Append(
		context.Background(),
		TestEvent("after-"+strings.Repeat("x", 80), now),
	); err != nil {
...
func TestRollingFileAppender_whenFileIndexMinConfigured_shouldReuseLowestIndex(t *testing.T) {
	...
	for index := 0; index < 5; index++ {
		if err := appender.Append(
			context.Background(),
			TestEvent("min-"+strings.Repeat("x", 80), now),
		); err != nil {
			t.Fatalf("Append(%d) error = %v", index, err)
		}
	}
...
func TestRollingFileAppender_whenDirectWriteConfigured_shouldWritePatternFilesDirectly(
	t *testing.T,
) {
	now := FixedTestTime()
	dir := t.TempDir()
	...
	for index := 0; index < 3; index++ {
		if err := appender.Append(
			context.Background(),
			TestEvent("direct-"+strings.Repeat("x", 80), now),
		); err != nil {
			t.Fatalf("Append(%d) error = %v", index, err)
		}
	}
...
func TestNewConfigured_whenYamlRollingFilePatternUsed_shouldArchiveWithPattern(t *testing.T) {
	...
	archivePath := filepath.Join(dir, "archive", "yaml-%d{yyyyMMdd}-%i.log.gz")
...
appenders:
  rolling:
    ...
    layout:
      type: text
    rolling:
      filePattern: "`+filepath.ToSlash(archivePath)+`"
      maxSize: 120
      maxBackups: 1
root:
  level: info
