...
func TestHandler_whenDefaultPatternUsed_shouldRenderSpringBootStyleLine(t *testing.T) {
	...
	record := slog.NewRecord(
		time.Date(2026, 8, 25, 10, 15, 30, 123000000, time.FixedZone("CST", 8*3600)),
		slog.LevelInfo,
		"service started",
		0,
	)
	record.AddAttrs(slog.String("profile", "dev"))
	if err := NewLogger(handler, "goark.dev.arkhos.hertz").Handler().Handle(
		context.Background(),
		record,
	); err != nil {
...
func TestHandler_whenNamedLoggerLevelIsLower_shouldUseMostSpecificRule(t *testing.T) {
	...
	handler, err := NewHandler(Options{
		Appenders: []Appender{
			NewConsoleAppender(
				WithConsoleName("root"),
				WithConsoleWriter(&rootOut),
				WithConsoleLayout(TextLayout{}),
			),
			NewConsoleAppender(
				WithConsoleName("orm"),
				WithConsoleWriter(&ormOut),
				WithConsoleLayout(TextLayout{}),
			),
		},
	...
	if strings.Contains(rootOut.String(), "sql prepared") {
		t.Fatalf(
			"non-additive named logger should not write root appender, got %q",
			rootOut.String(),
		)
	}
	if !strings.Contains(ormOut.String(), "sql prepared") ||
		!strings.Contains(ormOut.String(), "statement=FindByID") {
		t.Fatalf("named output should contain debug SQL line, got %q", ormOut.String())
	}
}

func TestHandler_whenAppenderRefControlConfigured_shouldApplyLevelAndFiltersPerAppender(
	t *testing.T,
) {
...
func TestHandler_whenUsedConcurrently_shouldKeepLinesComplete(t *testing.T) {
	...
	for _, line := range lines {
		if !strings.Contains(line, "msg=event") ||
			!strings.Contains(line, "logger=goark.concurrent") {
