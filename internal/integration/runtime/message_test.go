...
func TestMapMessage_whenAttrsProvided_shouldExposeMessageAndAttrs(t *testing.T) {
	...
	if got := message.String(); !strings.Contains(got, "user=alice") ||
...
func TestStructuredDataMessage_whenCreated_shouldExposeStructuredAttrs(t *testing.T) {
	message := NewStructuredDataMessage(
		"audit@32473",
		"login",
		"accepted",
	...
	if got := message.String(); !strings.Contains(got, "[audit@32473") ||
		!strings.Contains(got, "accepted") {
	...
	if attrs[0].Key != StructuredDataIDAttrKey || attrs[1].Key != StructuredDataTypeAttrKey ||
		attrs[2].Key != "user" {
