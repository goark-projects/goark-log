...
func TestAsyncAppender_whenErrorHandlerConfigured_shouldReceiveWriteFailure(t *testing.T) {
	...
	appender, err := NewAsyncAppender(
		[]Appender{NewFailingAppender("broken")},
		WithAsyncQueueSize(4),
		WithAsyncErrorHandler(
			AsyncErrorHandlerFunc(func(_ context.Context, err error, event Event) {
	...
	if err := appender.Append(
		context.Background(),
		TestEvent("broken event", FixedTestTime()),
	); err != nil {
