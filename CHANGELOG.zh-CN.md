...
- 新增 Spring Boot 风格控制台布局、Logger 名称缩写、结构化 JSON 格式、可配置
  输出字符集和运行时日志级别控制。
- 新增内存配置加载和已加载选项定制器，Boot 集成无需临时文件。
- 加强启动、按大小滚动和保留策略行为。

### 变更

- 根包改名为 `log`，与模块的公共标识保持一致。
...
- 默认控制台日志改为写入 stdout。
- 即使文件元数据没有变化，配置重载也能检测内容变化。
- Boot 属性可用于日志 lookup，结构化堆栈跟踪输出遵循配置语义。
